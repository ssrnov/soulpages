...
$PAGE_SUB   = 'Live device status, current app & location';
$PAGE_ICON  = '📍';
...
$rows = $pdo->query(
    "SELECT u.id, u.username, u.name, u.is_online, u.last_seen, u.presence_at,
     ...
     WHERE u.role='user' AND u.id=$uid
      ...
      <div style="grid-column:1 / -1">
        ...
        <div style="display:flex;align-items:center;flex-wrap:wrap;gap:8px">
          ...
          <?php if($r['loc']): ?>
            ...
            <span style="color:var(--mut);font-size:.7rem"><?= $r['loc_at']?date('d M H:i',strtotime($r['loc_at'])):'' ?><?php if($r['loc_acc']!==null && (float)$r['loc_acc']>0): ?> · ±<?= (int)round((float)$r['loc_acc']) ?>m<?php endif; ?></span>
