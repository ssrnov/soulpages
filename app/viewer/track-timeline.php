...
$PAGE_TOOLS = '<form method="get" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap"><input type="hidden" name="user" value="'.$uid.'"><input class="sel" type="date" name="day" value="'.ss_admin_h($day).'" max="'.date('Y-m-d').'" onchange="this.form.submit()"></form>';
...
<div class="panel">
  ...
  <div style="font-size:.75rem;color:var(--mut);margin:6px 0 12px">Per-minute — which app was on, merged from <b>foreground</b> (📱) and <b>network activity</b> (📡, catches hidden apps). e.g. "10:03 → 10:25 · Instagram".</div>
  <?php if(!$timeline): ?>
