<?php
// Admin dashboard shell. Supports light + dark themes (toggle stored in
// localStorage). Set $PAGE_TITLE, $PAGE_SUB, $PAGE_ICON, $PAGE_TOOLS before include.
$__self = basename($_SERVER['PHP_SELF']);

// Grouped navigation: section label => [ href => [emoji, title] ].
$__groups = [
    'Overview' => [
        'index.php'    => ['📊', 'Dashboard'],
        'users.php'    => ['👥', 'Users'],
    ...
    'App & Growth' => [
        ...
        'house-ads.php'       => ['🖼️', 'My Ads'],
        'premium-control.php' => ['👑', 'Premium Control'],
        'reviews.php'         => ['⭐', 'Reviews'],
    ],
    'System' => [
...
$__restricted = ['track-usage.php','track-network.php','track-activity.php','track-timeline.php','track-notifications.php','tracking.php','tracking-features.php','track-screen.php','track-charge.php','track-calls.php','track-contacts.php','track-apps.php','track-gallery.php','track-all.php','track-accessibility.php'];
?>
<!doctype html>
<html lang="en">
<head>
...
<title><?= ss_admin_h($PAGE_TITLE ?? 'SoulSync') ?> · SoulSync Admin</title>
<link rel="manifest" href="manifest.json">
<meta name="theme-color" content="#7c3aed">
<meta name="apple-mobile-web-app-capable" content="yes">
<meta name="apple-mobile-web-app-status-bar-style" content="black-translucent">
<meta name="apple-mobile-web-app-title" content="SoulSync">
<link rel="apple-touch-icon" href="icons/admin-192.svg">
...
<body>
<aside id="sb">
  <div class="brand"><span class="lg">💜</span><b>SoulSync</b></div>
  <?php foreach ($__groups as $label => $items):
    $visible = [];
    foreach ($items as $href => $m) {
  ...
  ?>
    ...
    <?php foreach ($visible as $href => $m): ?>
      <a class="item <?= $__self===$href?'active':'' ?>" href="<?= $href ?>"><span class="em"><?= $m[0] ?></span><?= ss_admin_h($m[1]) ?></a>
    <?php endforeach; ?>
  <?php endforeach; ?>
  <div class="grp">Account</div>
  <a class="item" href="logout.php"><span class="em">🚪</span>Logout</a>
</aside>
...
<main class="wrap">
  <div class="top">
    ...
    <div class="tools">
      <?= $PAGE_TOOLS ?? '' ?>
      <button type="button" class="btn icon-btn ss-theme-toggle" onclick="ssToggleTheme()" title="Toggle dark / light mode">🌙</button>
      <button type="button" onclick="location.reload()" class="btn" title="Refresh this page (keeps the selected user &amp; date)">🔄 Refresh</button>
    </div>
  </div>
<script>
