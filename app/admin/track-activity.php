...
// guessed from the data rate (Watching Reels vs Texting, etc.). Super admin only.
...
// Colour + icon per activity label.
...
// Pull every labelled per-minute slot for the day, newest-app-grouped.
$rows = [];
try {
...
// Group consecutive same (app+label) minutes into ranges.
$acts = [];
$GAP = 180; $CYCLE = 60;
...
// Minute-by-minute rows (raw), newest first — the exact "ek minute ka hisaab".
$minutes = $rows;
usort($minutes, fn($a,$b)=> strtotime($b['active_at']) <=> strtotime($a['active_at']));

// Totals per activity type for a quick summary.
$byLabel = [];
foreach ($rows as $r) { $byLabel[$r['label']] = ($byLabel[$r['label']] ?? 0) + $CYCLE; }
arsort($byLabel);

$PAGE_TITLE = 'In-app Activity';
$PAGE_SUB   = 'What each user was doing inside apps — Reels vs Texting, minute by minute';
$PAGE_ICON  = '🎬';
